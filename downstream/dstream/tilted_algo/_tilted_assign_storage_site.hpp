...
std::optional<uint64_t> tilted_assign_storage_site(uint64_t S, uint64_t T) {
    ...
    uint64_t t_1 = (uint64_t{1} << (tau + 1)) - (tau + 1);  // Opening epoch of next meta-epoch
    bool epsilon_b = t < h + t_0 && h + t_0 < t_1;  // Uninvaded correction factor
    uint64_t B = (S >> (tau + 1 - epsilon_b)) ? (S >> (tau + 1 - epsilon_b)) : 1;  // Num bunches available to h.v.
    ...
    uint64_t p = b_l - std::bit_floor(b_l);  // Bunch position within nestedness level
