#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_11_STEADY_12__ASSIGN_STORAGE_SITE_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_11_STEADY_12__ASSIGN_STORAGE_SITE_HPP
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(dstream_hybrid_0_circular_11_steady_12::has_ingest_capacity<UINT>(S, T));

  const UINT frac_S = S / 12;
  const UINT circ_S = 11 * frac_S;
  const UINT remainder = T % 12;
  if (remainder < 11) {
    const UINT adj_T = (T / 12) * 11 + remainder;
    const UINT site =
        dstream_circular::_assign_storage_site<UINT>(circ_S, adj_T);
    return (site == circ_S) ? S : site;
  } else {
    const UINT adj_T = T / 12;
    const UINT site =
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_circular_11_steady_12::_assign_storage_site<UINT>(S, T);
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_11_STEADY_12__ASSIGN_STORAGE_SITE_HPP
