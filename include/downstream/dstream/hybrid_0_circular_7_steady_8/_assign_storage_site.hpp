#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_7_STEADY_8__ASSIGN_STORAGE_SITE_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_7_STEADY_8__ASSIGN_STORAGE_SITE_HPP
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(dstream_hybrid_0_circular_7_steady_8::has_ingest_capacity<UINT>(S, T));

  const UINT frac_S = S / 8;
  const UINT circ_S = 7 * frac_S;
  const UINT remainder = T % 8;
  if (remainder < 7) {
    const UINT adj_T = (T / 8) * 7 + remainder;
    const UINT site =
        dstream_circular::_assign_storage_site<UINT>(circ_S, adj_T);
    return (site == circ_S) ? S : site;
  } else {
    const UINT adj_T = T / 8;
    const UINT site =
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_circular_7_steady_8::_assign_storage_site<UINT>(S, T);
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_CIRCULAR_7_STEADY_8__ASSIGN_STORAGE_SITE_HPP
