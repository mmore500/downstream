...
/**
 * Internal impl of site selection for hybrid steady/stretched curation.
 *
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(
      dstream_hybrid_0_steady_1_stretched_2::has_ingest_capacity<UINT>(S, T));
  ...
  if ((T & 1) == 0) {
    ...
    return half_S +
           dstream_stretched::_assign_storage_site<UINT>(half_S, half_T);
  }
}

/**
 * Site selection algorithm for hybrid steady/stretched curation.
 *
 * @tparam UINT Unsigned integer type for operands and return value.
 * @param S Buffer size.
 *     Must be a power of two, greater than 2.
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_steady_1_stretched_2::_assign_storage_site<UINT>(S, T);
