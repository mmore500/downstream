#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_1_TILTED_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_1_TILTED_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
...
/**
 * Internal implementation of site selection for hybrid
 * steady/tilted/circular curation.
 *
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(
      dstream_hybrid_0_steady_1_tilted_2_circular_3::has_ingest_capacity<UINT>(
          S, T));
  ...
  if (remainder == 0) {
    const UINT site =
        dstream_steady::_assign_storage_site<UINT>(third_S, adj_T);
    return (site == third_S) ? S : site;
  } else if (remainder == 1) {
    const UINT site =
        dstream_tilted::_assign_storage_site<UINT>(third_S, adj_T);
    return (site == third_S) ? S : third_S + site;
  } else {
    const UINT site =
        dstream_circular::_assign_storage_site<UINT>(third_S, adj_T);
    return (site == third_S) ? S : 2 * third_S + site;
  }
}

/**
 * Site selection algorithm for hybrid steady/tilted/circular curation.
 *
 * @tparam UINT Unsigned integer type for operands and return value.
 * @param S Buffer size.
 *     Must be divisible by 3, with S/3 being a power of two.
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_steady_1_tilted_2_circular_3::_assign_storage_site<UINT>(
          S, T);
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_1_TILTED_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
