#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
...
namespace dstream_hybrid_0_steady_2_circular_3 {

/**
 * Internal implementation of site selection for hybrid steady/circular
 * curation.
 *
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(dstream_hybrid_0_steady_2_circular_3::has_ingest_capacity<UINT>(S, T));
  ...
  if (remainder < 2) {
    ...
    const UINT site =
        dstream_steady::_assign_storage_site<UINT>(two_thirds_S, adj_T);
...
/**
 * Site selection algorithm for hybrid steady/circular curation.
 *
 * @tparam UINT Unsigned integer type for operands and return value.
 * @param S Buffer size.
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_steady_2_circular_3::_assign_storage_site<UINT>(S, T);
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_CIRCULAR_3__ASSIGN_STORAGE_SITE_HPP
