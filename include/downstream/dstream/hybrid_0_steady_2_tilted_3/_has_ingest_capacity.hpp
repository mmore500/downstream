#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_TILTED_3__HAS_INGEST_CAPACITY_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_TILTED_3__HAS_INGEST_CAPACITY_HPP
...
bool has_ingest_capacity(const UINT S, const UINT T) {
  ...
  const bool has_capacity_1st =
      dstream_steady::has_ingest_capacity<UINT>(
          two_thirds_S, T_div_3 * 2 + static_cast<UINT>(T % 3 > 0));
  const bool has_capacity_2nd =
      ((T < 2) or
       dstream_tilted::has_ingest_capacity<UINT>(third_S, (T - 2) / 3));
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_STEADY_2_TILTED_3__HAS_INGEST_CAPACITY_HPP
