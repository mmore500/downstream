#pragma once
#ifndef DOWNSTREAM_DSTREAM_HYBRID_0_TILTED_1_CIRCULAR_2__ASSIGN_STORAGE_SITE_HPP
#define DOWNSTREAM_DSTREAM_HYBRID_0_TILTED_1_CIRCULAR_2__ASSIGN_STORAGE_SITE_HPP
...
/**
 * Internal implementation of site selection for hybrid tilted/circular curation.
 *
...
UINT _assign_storage_site(const UINT S, const UINT T) {
  assert(dstream_hybrid_0_tilted_1_circular_2::has_ingest_capacity<UINT>(S, T));

  const UINT half_S = S >> 1;
  const UINT half_T = T >> 1;
  if ((T & 1) == 0) {
    const UINT site =
        dstream_tilted::_assign_storage_site<UINT>(half_S, half_T);
    return (site == half_S) ? S : site;
  } else {
    return half_S + dstream_circular::_assign_storage_site<UINT>(half_S, half_T);
  }
}
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site =
      dstream_hybrid_0_tilted_1_circular_2::_assign_storage_site<UINT>(S, T);
...
#endif  // DOWNSTREAM_DSTREAM_HYBRID_0_TILTED_1_CIRCULAR_2__ASSIGN_STORAGE_SITE_HPP
