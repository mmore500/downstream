...
UINT _assign_storage_site(const UINT S, const UINT T) {
  ...
  const UINT t_0 = (_1 << tau) - tau;  // Opening epoch of meta-epoch
  const UINT t_1 =
      (_1 << (tau + _1)) - (tau + _1);  // Opening epoch of next meta-epoch
  const bool epsilon_b =
      t < h + t_0 && h + t_0 < t_1;  // Uninvaded correction factor
  const UINT B = std::max<UINT>(S >> (tau + _1 - epsilon_b), _1);
  ...
  const UINT k_b = (b_p << _1) + aux::popcount_casted<UINT>((S << _1) - b_p) -
                   _1 - epsilon_k_b;  // Site index of bunch
...
std::optional<UINT> assign_storage_site(const UINT S, const UINT T) {
  const UINT site = dstream_tilted::_assign_storage_site<UINT>(S, T);
  return std::optional<UINT>(site);
}
