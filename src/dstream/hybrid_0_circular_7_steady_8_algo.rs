...
pub fn has_ingest_capacity<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> bool {
    ...
    let t_div_8: Uint = T / _8;
    let t_mod_8: Uint = T % _8;
    let adj_t_mod: Uint = if t_mod_8 < _7 { t_mod_8 } else { _6 };
    let has_capacity_1st =
        CircularAlgo::has_ingest_capacity(seven_eighth_S, t_div_8 * _7 + adj_t_mod);
    let has_capacity_2nd = T < _7 || SteadyAlgo::has_ingest_capacity(eighth_S, (T - _7) / _8);
...
pub fn _assign_storage_site<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> Uint {
    ...
    if remainder < _7 {
        let adj_T: Uint = (T / _8) * _7 + remainder;
        let site: Uint = CircularAlgo::_assign_storage_site(seven_eighth_S, adj_T);
        if site == seven_eighth_S {
            S
        } else {
            site
        }
        ...
        let adj_T: Uint = T / _8;
        let site: Uint = SteadyAlgo::_assign_storage_site(eighth_S, adj_T);
        if site == eighth_S {
            S
        } else {
            seven_eighth_S + site
