use super::{AssignStorageSiteTrait, CircularAlgo, HasIngestCapacityTrait, TiltedAlgo};
...
pub fn has_ingest_capacity<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> bool {
    let half_S: Uint = S >> 1;
    let has_capacity_1st = TiltedAlgo::has_ingest_capacity(half_S, T >> 1);

    let has_capacity_2nd =
        T == Uint::zero() || CircularAlgo::has_ingest_capacity(half_S, (T - Uint::one()) >> 1);
...
pub fn _assign_storage_site<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> Uint {
    ...
    if T & Uint::one() == Uint::zero() {
        let site: Uint = TiltedAlgo::_assign_storage_site(half_S, half_T);
        if site == half_S {
            S
        } else {
            site
        ...
        half_S + CircularAlgo::_assign_storage_site(half_S, half_T)
    }
}
