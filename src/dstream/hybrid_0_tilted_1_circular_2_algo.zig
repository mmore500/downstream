...
pub fn has_ingest_capacity(comptime u: type, S: u, T: u) bool {
    ...
    const has_capacity_2nd = ((T == 0) or circular_algo.has_ingest_capacity(
        u,
        half_S,
        (T - 1) >> 1,
    ));
...
pub fn assign_storage_site(comptime u: type, S: u, T: u) u {
    ...
    const half_T = T >> 1;
    if (T & 1 == 0) {
        const site = tilted_algo.assign_storage_site(u, half_S, half_T);
        return if (site == half_S) S else site;
    } else {
        return half_S + circular_algo.assign_storage_site(u, half_S, half_T);
    }
}
