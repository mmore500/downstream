...
pub fn has_ingest_capacity<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> bool {
    ...
    let has_capacity_1st = TiltedAlgo::has_ingest_capacity(
        two_thirds_S,
        t_div_3 * _2 + aux::from_bool::<Uint>(T % _3 > _0),
    );
    let has_capacity_2nd = T < _2 || CircularAlgo::has_ingest_capacity(third_S, (T - _2) / _3);
...
pub fn _assign_storage_site<Uint: aux::UnsignedTrait>(S: Uint, T: Uint) -> Uint {
    ...
    if remainder < _2 {
        ...
        let site: Uint = TiltedAlgo::_assign_storage_site(two_thirds_S, adj_T);
