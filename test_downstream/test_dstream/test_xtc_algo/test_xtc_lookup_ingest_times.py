...
time_lookup = validate_xtc_time_lookup(algo.lookup_ingest_times)


@pytest.mark.parametrize("s", range(3, 12))
def test_xtc_time_lookup_against_site_selection(s: int):
    S = 1 << s
...
def test_xtc_time_lookup_fuzz(S: int, T: int):
